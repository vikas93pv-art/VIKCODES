export default function ExpenseTracker() {
  ...
  // NEW: Normalize ID handling to fix edit/delete
  const normalizeExpense = (e, index) => {
    const rawId = e.id || e.ID;
    if (!rawId) {
      console.warn('Expense missing ID:', e);
    }
    return {
      id: String(rawId || `temp-${index}-${Date.now()}`),
  ...
  // FIXED: Fetch with better error handling
  const fetchExpenses = async () => {
    ...
    try {
      ...
      console.log('Fetched expenses:', data.length, 'records');
  ...
  // FIXED: Handle edit with ID validation
  const handleOpenEdit = (expense) => {
    console.log('Opening edit for expense:', expense);
    
    if (!expense.id || expense.id.startsWith('temp-')) {
      showToast('Error: Cannot edit expense without valid ID', 'error');
      return;
    }
  ...
  // FIXED: Handle update with robust error handling
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingExpense) {
        const expenseId = String(editingExpense.id).trim();
        const url = `${SUPABASE_URL}/rest/v1/Expenses?id=eq.${encodeURIComponent(expenseId)}`;
      ...
      if (!res.ok) {
        ...
        if (res.status === 401 || res.status === 403) {
          throw new Error('Permission denied. Check Supabase RLS policies for UPDATE/DELETE.');
        }
        if (res.status === 404) {
          throw new Error('Expense not found. It may have been deleted.');
        }
        
        throw new Error(`${editingExpense ? 'Update' : 'Add'} failed: ${errorText}`);
      }
      
      await fetchExpenses();
      handleCloseModal();
      showToast(editingExpense ? 'Expense updated successfully!' : 'Expense added!', 'success');
  ...
  // FIXED: Delete with ID validation and better errors
  const handleDeleteConfirm = (id) => {
    if (!id || String(id).startsWith('temp-')) {
      showToast('Error: Cannot delete expense without valid ID', 'error');
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401 || res.status === 403) {
          throw new Error('Permission denied. Enable DELETE policy in Supabase for anon users.');
        }
        if (res.status === 404) {
          throw new Error('Expense already deleted or not found.');
        }
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-neutral-900/60 backdrop-blur-md">
            <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-[0_50px_100px_rgba(0,0,0,0.3)] overflow-hidden animate-in zoom-in-95 duration-300">
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-8">
                ...
                >
                  {isSyncing ? 'Processing...' : (editingExpense ? 'Update Transaction' : 'Commit Transaction')}
                </button>
              </form>
            </div>
          </div>
...
}// REPLACE your entire ExpenseTracker component with this:
...
const Toast = ({ message, type, onClose }) => {
  useEffect(() => {
    const timer = setTimeout(onClose, 5000); // Increased to 5s for debugging
  ...
  return (
    <div className={`fixed top-6 right-6 z-[600] ${bgColor} text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-5 duration-300 max-w-md`}>
      <Icon className="w-5 h-5 flex-shrink-0" />
      <span className="font-bold text-sm break-words">{message}</span>
      <button onClick={onClose} className="ml-2 hover:bg-white/20 rounded-lg p-1 transition-colors flex-shrink-0">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
...
export default function ExpenseTracker() {
  ...
  // CRITICAL FIX: Normalize ID handling to ensure string comparison
  const normalizeExpense = (e, index) => {
    // Handle both 'id' and 'ID' from Supabase, ensure it's a string
    const rawId = e.id || e.ID;
    if (!rawId) {
      console.warn('Expense missing ID:', e);
    }
    return {
      id: String(rawId || `temp-${index}-${Date.now()}`), // Ensure string ID
      amount: parseFloat(e.amount || 0),
      merchant: e.merchant || 'Unknown',
      category: e.category || 'Other',
      paymentMode: e.paymentmode || e.paymentMode || 'Other',
    ...
    reader.onload = async (e) => {
      ...
      if (newExpenses.length > 0) {
        ...
        try {
          ...
          // CRITICAL FIX: Better error handling for import
          if (!res.ok) {
            const errorText = await res.text();
            throw new Error(`Import failed: ${errorText}`);
          }
  ...
  // CRITICAL FIX: Open edit with validation
  const handleOpenEdit = (expense) => {
    console.log('Opening edit for expense:', expense);
  ...
  // CRITICAL FIX: Handle Submit with proper ID handling and error reporting
  const handleSubmit = async (e) => {
    e.preventDefault();
    ...
    const payload = { 
      ...
      merchant: merchant.trim(), 
      category, 
      paymentmode: paymentMode, // Note: lowercase to match typical Supabase column naming
      date 
    };
    ...
    try {
      ...
      if (editingExpense) {
        // CRITICAL: Ensure we're using the string ID and proper URL encoding
        const expenseId = String(editingExpense.id).trim();
        const url = `${SUPABASE_URL}/rest/v1/Expenses?id=eq.${encodeURIComponent(expenseId)}`;
      ...
      // CRITICAL FIX: Detailed error reporting
      if (!res.ok) {
        const errorText = await res.text();
        let errorData;
        try {
        ...
        if (res.status === 401 || res.status === 403) {
          throw new Error('Permission denied. Check Supabase RLS policies (enable UPDATE/DELETE for anon role)');
        }
        if (res.status === 404) {
          throw new Error(`Expense not found (ID may be invalid or already deleted). ${errorData.message || ''}`);
        }
        
        throw new Error(`${editingExpense ? 'Update' : 'Add'} failed: ${errorData.message || errorText}`);
      }
  ...
  // CRITICAL FIX: Delete confirmation with ID validation
  const handleDeleteConfirm = (id) => {
    if (!id || String(id).startsWith('temp-')) {
      showToast('Error: Cannot delete expense without valid ID', 'error');
      return;
  ...
  // CRITICAL FIX: Delete handler with proper error reporting
  const handleDelete = async () => {
    const { expenseId } = confirmDialog;
    setConfirmDialog({ isOpen: false, expenseId: null });
    
    if (!expenseId) {
      showToast('No expense ID provided for deletion', 'error');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          throw new Error('Expense not found. It may have been already deleted.');
        }
        
        throw new Error(`Delete failed: ${errorData.message || errorText}`);
      }
